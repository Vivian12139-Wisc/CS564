 ...
 * Name: Yuxin Huang; Id: 9073788490
 * Name: Xunwei Ye;   Id: 9081588783
 *
 * Purpose of file: 
 * This file serves as the implementation of all methods in the BufMgr class, which is the heart of the buffer manager. 
 * The buffer manager controls which pages are memory resident. Whenever the buffer manager receives a request for a data page, 
 * the buffer manager checks to see if the requested page is already in the one of the frames that constitutes the buffer pool. 
 * If so, the buffer manager simply returns a pointer to the page. If not, the buffer manager frees a frame (possibly by writing 
 * to disk the page it contains if the page is dirty) and then reads in the requested page from disk into the frame that has been freed.
 *
 * @section LICENSE
...
void BufMgr::allocBuf(FrameId & frame) 
{
    ...
    // this happens when all pages are valid, with pinCnt >= 1
